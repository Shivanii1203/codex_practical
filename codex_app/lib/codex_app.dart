import 'package:codex_app/java/javatopics.dart';
import 'package:flutter/material.dart';

class Demo extends StatelessWidget {
   Demo({super.key});
    List<String> Javatopic=["Introduction","Operators","Array","String","Inheritence","Interface","Polymorphism","Abstract"];
    List<String>  Darttopic=["Introduction","Operators","Inheritence","Interface","Polymorphism","Abstract"];
   

   
  
     Widget reusableContainer(BuildContext context,String name,String imgurl,List topiclist,){
    return InkWell(
      onTap: () {
        
        Navigator.push(context, MaterialPageRoute(builder: (context)=>Javatopics(name:name,topic:topiclist,)),);
      },
      child: Container(
        
                height: 250,
                width: 350,
                            
                            margin: EdgeInsets.only(top:20,left: 50,bottom: 10,right: 20),
                            decoration: BoxDecoration(
                              image: DecorationImage(image: NetworkImage(imgurl),fit: BoxFit.fill),
                              color: Colors.white,
                              boxShadow:[
                                BoxShadow(
                                  color:  Colors.black,
                                  offset: Offset(10, 10),
                                  blurRadius: 10,
                                ),
                                ]
                            ),
                          
                        ),
                        
    );
                     
  
   }

  
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Codex", style: TextStyle(color: Colors.black)),
        actions: [
          IconButton(
            onPressed: () {
              print("click on add");
            },
            icon: Icon(Icons.search, color: Color.fromARGB(255, 10, 10, 10)),
            tooltip: "search",
            color: Colors.blue,
          ),
        ],
      ),
      body:SizedBox(
        child: Container(
          width: double.infinity,
          height: double.infinity,
         
          
          child: SingleChildScrollView(
          scrollDirection:Axis.vertical,
        
        child:Column(
          
          crossAxisAlignment: CrossAxisAlignment.start,
          children:[
            SizedBox(height: 20),
             
             Container(
         margin:EdgeInsets.only(top: 50 , left: 50),
            child:Text ("Languages",style:TextStyle(fontWeight: FontWeight.w900,fontSize: 25,)
            ),),
            SizedBox(height: 20),
    
            Container(
              child:SingleChildScrollView(
               scrollDirection: Axis.horizontal,
              
                       child:Row(
              
              children: [
                reusableContainer(context,"Java","https://www.simplilearn.com/ice9/free_resources_article_thumb/Java_classes_and_objects.jpg",Javatopic),
                 reusableContainer(context,"Dart","https://img-b.udemycdn.com/course/750x422/5514568_55e3_2.jpg",Darttopic),
                  //  reusableContainer(context,"Javascript","data:image/png;base64,",JavaScripttopic),
                  //   reusableContainer(context,"Html","https://media.licdn.com/dms/image/D4D12AQEaTK5_cv5Fmg/article-cover_image-shrink_720_1280/0/1677082453584?e=2147483647&v=beta&t=hy4wcIl2vOpp5lr5k74obYVN63IT-U4lydNOIo3hHGo",Htmltopic),    
              ],
              //          
                  ),
                    
            
           
              ),
              ),

        //           Container(
        //  margin:EdgeInsets.only(top: 50 , left: 50),
        //     child:Text ("Framework",style:TextStyle(fontWeight: FontWeight.w900,fontSize: 25,)
        //     ),),
        //     SizedBox(height: 20),
    
        //     Container(
            
        //       child:SingleChildScrollView(
        //        scrollDirection: Axis.horizontal,
                      
                      
        //        child:Row(
        //       children: [
        //         reusableContainer(context,"React.js","https://blog.talent500.co/wp-content/uploads/2023/10/Benefits-of-ReactJS.jpg",Reacttopic,Reactcodes),
        //          reusableContainer(context,"Hibernate","https://media.licdn.com/dms/image/D5612AQEiS-O7TkXquQ/article-cover_image-shrink_720_1280/0/1693368581608?e=2147483647&v=beta&t=AZAMVOOWq1yuU72KC5ywRTBZT7VAMIYA8HT25jhkRoo",Hibernatetopic,Hibernatecodes),
        //           reusableContainer(context,"Spring","https://cdn.servermania.com/images/f_webp,q_auto:best/v1693496622/blog/Featured-4/Featured-4.png?_i=AA",Springtopic,Springcodes),
        //            reusableContainer(context,"Angular","https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAmfcxQUYkAKaaRpT4edstSaAxhynHaPpwWg&usqp=CAU",Angulartopic,Angularcodes),    
        //       ],
        //           ),
        //       ),
        //       ),

        //           Container(
        //  margin:EdgeInsets.only(top: 50 , left: 50),
        //     child:Text ("Databases",style:TextStyle(fontWeight: FontWeight.w900,fontSize: 25,)
        //     ),),
        //     SizedBox(height: 20),
    
        //     Container(
        //       child:SingleChildScrollView(
        //        scrollDirection: Axis.horizontal,
              
        //                child:Row(
              
        //       children: [
        //         reusableContainer("https://cms.intuji.com/wp-content/uploads/2023/10/MySQL.png"),
        //          reusableContainer("https://discover.strongdm.com/hubfs/Imported_Blog_Media/60cbc1205c30f372af2801ce_create-user-mongodb-securely-3.jpeg"),
        //           reusableContainer("data:image/jpeg;base64,"),
        //            reusableContainer("https://i0.wp.com/teclues.com/wp-content/uploads/2020/08/mongo.jpg"),    
        //       ],      
        //           ),
        //       ),
        //       ),

        //           Container(
        //  margin:EdgeInsets.only(top: 50 , left: 50),
        //     child:Text ("Tools",style:TextStyle(fontWeight: FontWeight.w900,fontSize: 25,)
        //     ),),
        //     SizedBox(height: 20),
    
        //     Container(
        //       child:SingleChildScrollView(
        //        scrollDirection: Axis.horizontal,
              
        //                child:Row(
              
        //       children: [
        //         reusableContainer("https://static-00.iconduck.com/assets.00/git-icon-2048x2048-r06ixacl.png"),
        //          reusableContainer("https://miro.medium.com/v2/resize:fit:3000/0*r30odGcXiuapymH4"),
        //           reusableContainer("https://strapi.dhiwise.com/uploads/618fa90c201104b94458e1fb_64fec58f7275d4c0f238ad3a_must_have_extensions_for_vs_code_Main_Image_b28ba3f4bb.jpg"),
        //            reusableContainer("https://www.tabnine.com/blog/wp-content/uploads/2021/11/blog_image_19-1.png"),    
        //       ],      
        //           ),
        //       ),
        //       ),
          ],
        ),
        ),
        ), 
      ),
    );
  }
}





























